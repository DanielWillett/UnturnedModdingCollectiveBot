...
public interface IPersistingRoleService
{
    ...
    Task<IReadOnlyList<PersistingRole>> GetPersistingRoles(ulong user, ulong guildId, ulong roleId, CancellationToken token = default);
    Task<IReadOnlyList<PersistingRole>> GetPersistingRoles(ulong user, ulong guildId, CancellationToken token = default);
